...
\includepdf[pages=-]{content/pdfs/code-of-conductmd.pdf}
...
Operators are depicted using a rectangular box \raisebox{-3pt}{\tikz{\node [shape=rectangle, draw, line width=0.3mm, inner sep=0.8mm, font=\footnotesize] {operator}}} labeled with the operators name.
...
An event projected to the target observable is shown using a vertical arrow \raisebox{-3pt}{\tikz[scale=0.36]{\draw[line width=0.4mm, gray, dotted] (0,1) edge[->] (0,0)}} pointing from the operator to the target observable.
...
A marble diagram showing a source observable emitting the integers from 1 through 6. The source completes immediately after its last value was emitted. The \texttt{take}\footnote{\url{https://rxjs.dev/api/operators/take}} operator projects the first four values and then completes the target observable.
