\begin{figure}[H]
  ...
  \begin{tikzpicture}[node distance=6mm]
    ...
    \begin{pgfonlayer}{nodelayer}
        ...
        \node (filter) [style=operator, below=13mm of 1.west, anchor=west] {filter(i => i \% 2 === 0)};
        ...
        \node (map) [style=operator, below=38mm of 1.west, anchor=west] {map(i => i + 1)};
    ...
    \begin{pgfonlayer}{edgelayer}
        ...
        \draw (2_f)                 edge[style=project]     ($(2_f) -(0,8.5mm)$);
        \draw (4_f)                 edge[style=project]     ($(4_f) -(0,8.5mm)$);
        \draw (6_f)                 edge[style=project]     ($(6_f) -(0,8.5mm)$);
        \draw (8_f)                 edge[style=project]     ($(8_f) -(0,8.5mm)$);
    ...
    \end{pgfonlayer}
  \end{tikzpicture}
  \caption[Marble diagram visualizing an observable]{
    The marble diagram for Listing \ref{lst:example-rxjs} shows three observables from top to bottom: The input
    observable emitting integers from 1 to 8, the intermediate result observable of the
    \texttt{filter} operator emitting only even integers, and the output observable emitting the even integers ``plus 1''.
    The two operators are shown in between the observables. The vertical line through the last marble of the
    input observable indicates that the input observable completed. Both operators forwarded this event to the output
    accordingly.
  }
